...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        paths_code += "        is_diffusion = False; is_gguf = False; first_gguf = None\n"
        ...
        paths_code += "                try:\n"
        paths_code += "                    ggufs = []\n"
        paths_code += "                    for root, dirs, fns in safe_walk(sf):\n"
        paths_code += "                        for fn in fns:\n"
        paths_code += "                            if fn.endswith('.gguf'): ggufs.append(os.path.join(root, fn))\n"
        paths_code += "                    if ggufs:\n"
        paths_code += "                        is_gguf = True\n"
        paths_code += "                        split = sorted([x for x in ggufs if '-00001-of-' in os.path.basename(x)])\n"
        paths_code += "                        first_gguf = (split or sorted(ggufs))[0]\n"
        paths_code += "                except Exception: pass\n"
        paths_code += "        models.append({'repo_id':rid,'size_bytes':sz,'nb_files':nf,'has_incomplete':ic,'path':cache,'is_diffusion':is_diffusion,'is_gguf':is_gguf,'gguf_path':first_gguf})\n"
        ...
        paths_code += "        is_model = False; is_gguf = False; first_gguf = None; ggufs = []\n"
        paths_code += "        for root, dirs, fns in safe_walk(fp):\n"
        paths_code += "            for fn in fns:\n"
        paths_code += "                if fn.endswith('.gguf'):\n"
        paths_code += "                    is_gguf = True; is_model = True; ggufs.append(os.path.join(root, fn))\n"
        paths_code += "                elif fn == 'config.json' or fn.endswith('.safetensors') or fn.endswith('.bin'): is_model = True\n"
        paths_code += "            if is_model and is_gguf: break\n"
        ...
        paths_code += "        models.append({'repo_id':d,'size_bytes':sz,'nb_files':nf,'has_incomplete':False,'path':p,'is_local_dir':True,'is_diffusion':is_diff,'is_gguf':is_gguf,'gguf_path':first_gguf})\n"
